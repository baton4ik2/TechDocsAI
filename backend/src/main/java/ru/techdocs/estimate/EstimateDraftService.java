...
public class EstimateDraftService {
    ...
    private static final double MODEL_MATCH_THRESHOLD = ru.techdocs.common.ModelMatching.THRESHOLD;
    ...
    private List<String> digits(String modelKey) {
        return ru.techdocs.common.ModelMatching.digits(modelKey);
    }
     ...
     * <p>
     * Минимальная длина ключа — 3, а не 4: «РМ-4» нормализуется в «pm4», и порог в
     * четыре знака молча выбрасывал такие модели из подбора — расценка по эталону
     * для них просто не предлагалась.
     */
    private List<EstimateDecisionService.EtalonOp> etalonOpsByModel(
            EstimateDecisionService.SystemEtalon etalon, String model) {
        EstimateDecisionService.EtalonType best = bestByModel(etalon.types(), model, 3);
        return best != null ? best.ops() : List.of();
    }

    private String modelKey(String model) {
        return ru.techdocs.common.ModelMatching.modelKey(model);
    }

    private double similarity(String a, String b) {
        return ru.techdocs.common.ModelMatching.similarity(a, b);
    }
