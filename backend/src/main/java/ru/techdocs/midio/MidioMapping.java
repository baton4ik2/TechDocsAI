...
public final class MidioMapping {
    ...
    /**
     * Работа про пожарную ЗОНУ, а не про изделие: «ТО „Рубеж Пожарная зона“».
     * Зона — сущность диспетчеризации, в смете обслуживания ей делать нечего,
     * такие работы при переносе пропускаются. Границы слова обязательны:
     * «сезонное» и «озонатор» зонами не являются.
     */
    private static final java.util.regex.Pattern ZONE_WORD =
            java.util.regex.Pattern.compile("(?iu)(?<!\\p{L})зон[а-яё]*(?!\\p{L})");

    public static boolean mentionsZone(String title) {
        return title != null && ZONE_WORD.matcher(title).find();
    }
